...
这里的 save/load 实现是基于 electron dialog 这个 API 来实现调用各类系统的文件保存提示框。这里我以 windows 系统为例来复现一下。
...
粗浅的看了一眼文档，dialog 能提供保存、读取以及提示对话框实现，而且还是系统样式的文件保存实现。以及还有异步和同步两种实现。
...
这里因为要读取数据，做个同步，等待数据回传到 Edit 组件这里。
